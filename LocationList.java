package VoterRegistrationSystem;

import com.itextpdf.io.font.FontConstants;
import com.itextpdf.kernel.events.Event;
import com.itextpdf.kernel.events.IEventHandler;
import com.itextpdf.kernel.events.PdfDocumentEvent;
import com.itextpdf.kernel.font.PdfFont;
import com.itextpdf.kernel.font.PdfFontFactory;
import com.itextpdf.kernel.geom.PageSize;
import com.itextpdf.kernel.pdf.PdfDocument;
import com.itextpdf.kernel.pdf.PdfWriter;
import com.itextpdf.layout.Document;
import com.itextpdf.layout.Style;
import com.itextpdf.layout.element.Paragraph;
import com.itextpdf.layout.element.Text;
import com.itextpdf.layout.property.TextAlignment;

import java.awt.*;
import java.io.File;
import java.io.IOException;
import java.sql.*;
import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;
import java.util.Objects;

public class LocationList
{
    static LocalDateTime thisDayAndTime = LocalDateTime.now();
    static DateTimeFormatter formatDate = DateTimeFormatter.ofPattern("MM/dd/yyyy");
    static DateTimeFormatter formatTime = DateTimeFormatter.ofPattern("HH:mm:ss");
    static String dateToday = thisDayAndTime.format(formatDate);
    static String timeToday = thisDayAndTime.format(formatTime);
    static int count = 0;
    static int page = 1;
    static int fontSize = 9;

    static String distType;
    static String headerIncluded = "";
    static String sortingMetric = "";
    static String columnName;

    public static void getLocation(String user, String startingPrecinct, String endingPrecinct, String district, boolean summaryOnly) throws IOException
    {
        Style normal = new Style();
        PdfFont font = PdfFontFactory.createFont(FontConstants.COURIER);
        normal.setFont(font).setFontSize(fontSize);

        if (Objects.equals(endingPrecinct, ""))
        {
            endingPrecinct = startingPrecinct;
        }

        if (!Objects.equals(district, ""))
        {
            columnName = switch (district)
            {
                case "" -> "";
                case "" -> "";
                case "" -> "";
                case "" -> "";
                case "" -> "";
                default -> "";
            };

            headerIncluded = switch (district)
            {
                case "" -> "";
                case "" -> "";
                case "" -> "";
                case "" -> "";
                case "" -> "";
                default -> "";
            };

            distType = "";
        }
        else
        {
            if (startingPrecinct.contains("1") || startingPrecinct.contains("2"))
            {
                distType = "";
                columnName = "";
                sortingMetric = "";
            }
            else
            {
                distType = "";
                columnName = "";
            }
        }

        if (!summaryOnly)
        {
            try
            {
                // creates a pdf document writer and pdf document
                PdfWriter pWriter = new PdfWriter("C:/Test/Location.pdf");
                PdfDocument pdf = new PdfDocument(pWriter);
                pdf.setDefaultPageSize(PageSize.A4.rotate());
                Document doc = new Document(pdf);
                doc.setMargins(0f, 0f, 0f, 0f);

                pdf.addEventHandler(PdfDocumentEvent.START_PAGE, new CommonFunctions.RotateEventHandler(PageSize.A4.rotate()));

                Text text1 = new Text("REPORT DATE  " + dateToday + "                                        FCCO VOTER PROCESSING SYSTEM                                                 PAGE         1\n");
                text1.setFontSize(fontSize);

                // Files ID to left side and Report Time to right side          LOCATION LIST
                Text text2 = new Text("USER  " + CommonFunctions.addPaddingR(user, 61) + "VOTING LOCATION LIST                                                     TIME  " + timeToday + "\n");
                text2.setFontSize(fontSize);

                // add date range
                Text text3 = new Text("INCLUDING ........:  " + headerIncluded + " FROM " + startingPrecinct + " TO " + endingPrecinct + " by " + distType + "\n");
                text3.setFontSize(fontSize);

                // column headings
                Text text4_1 = new Text("PRECINCT                   # LOCATION / PRECINCTS                                    CONTACT             PHONE #1      PHONE #2      DELIVERY     INVENTORY\n");
                text4_1.setFontSize(fontSize);


                Paragraph paragraph1 = new Paragraph();
                paragraph1.setTextAlignment(TextAlignment.LEFT);

                Paragraph paragraph2 = new Paragraph();
                paragraph2.setTextAlignment(TextAlignment.LEFT);

                paragraph1.add(text1).addStyle(normal);
                paragraph1.add(text2).addStyle(normal);
                paragraph1.add(text3).addStyle(normal);
                paragraph1.add(text4_1).addStyle(normal);
                paragraph2.add("-----------------------------------------------------------------------------------------------------------------------------------------------------------").addStyle(normal);

                doc.add(paragraph1);
                doc.add(paragraph2);

                getDataForReport(doc, startingPrecinct, endingPrecinct, user);

                doc.close();

                String filePath = "C:/Test/Location.pdf";
                File file = new File(filePath);

                Desktop.getDesktop().open(file);
            }
            catch (Exception e)
            {
                throw new RuntimeException(e);
            }
        }
        else
        {
            try
            {
                // creates a pdf document writer and pdf document
                PdfWriter pWriter = new PdfWriter("C:/Test/Location.pdf");
                PdfDocument pdf = new PdfDocument(pWriter);
                pdf.setDefaultPageSize(PageSize.A4.rotate());
                Document doc = new Document(pdf);
                doc.setMargins(0f, 0f, 0f, 0f);

                pdf.addEventHandler(PdfDocumentEvent.START_PAGE, new CommonFunctions.RotateEventHandler(PageSize.A4.rotate()));

                Text text1 = new Text("REPORT DATE  " + dateToday + "                                        FCCO VOTER PROCESSING SYSTEM                                                 PAGE         1\n");
                text1.setFontSize(fontSize);

                // Files ID to left side and Report Time to right side          LOCATION LIST
                Text text2 = new Text("USER  " + CommonFunctions.addPaddingR(user, 61) + "VOTING LOCATION LIST                                                     TIME  " + timeToday + "\n");
                text2.setFontSize(fontSize);

                // add date range
                Text text3 = new Text("INCLUDING ........:  " + headerIncluded + " FROM " + startingPrecinct + " TO " + endingPrecinct + "\n");
                text3.setFontSize(fontSize);

                Paragraph paragraph1 = new Paragraph();
                paragraph1.setTextAlignment(TextAlignment.LEFT);

                Paragraph paragraph2 = new Paragraph();
                paragraph2.setTextAlignment(TextAlignment.LEFT);

                paragraph1.add(text1).addStyle(normal);
                paragraph1.add(text2).addStyle(normal);
                paragraph1.add(text3).addStyle(normal);
                paragraph2.add("-----------------------------------------------------------------------------------------------------------------------------------------------------------").addStyle(normal);

                doc.add(paragraph1);
                doc.add(paragraph2);

                getSummaryReport(doc, startingPrecinct, endingPrecinct, user);

                doc.close();

                String filePath = "C:/Test/Location.pdf";
                File file = new File(filePath);

                Desktop.getDesktop().open(file);
            }
            catch (Exception e)
            {
                throw new RuntimeException(e);
            }
        }
    }

    public static void getSummaryReport(Document doc, String startingID, String endingID, String user) throws IOException
    {
        String[] voterData = new String[20];

        int totalChairs = 0;
        int totalTables = 0;
        int totalSigns = 0;

        Style normal = new Style();
        PdfFont font = PdfFontFactory.createFont(FontConstants.COURIER);
        normal.setFont(font).setFontSize(fontSize);

        if (Objects.equals(endingID, ""))
        {
            endingID = startingID;
        }

        try
        {
            Class.forName("");
            String url = "";
            Connection conn = DriverManager.getConnection(url);

            PreparedStatement stmt = conn.prepareStatement("SELECT " +
                    "[], " +
                    "[], " +
                    "[] " +
                    "FROM [] " +
                    "WHERE ["+ columnName +"] BETWEEN '" + startingID + "' AND '" + endingID + "' ");

            ResultSet rs = stmt.executeQuery();

            while (rs.next())
            {
                voterData[0] = rs.getString(1); 
                voterData[1] = rs.getString(2);  
                voterData[2] = rs.getString(3);   

                for (int i = 0; i < 3; i++)
                {
                    if (voterData[i] == null)
                    {
                        voterData[i] = "0";
                    }
                }

                totalChairs += Integer.parseInt(voterData[0]);
                totalTables += Integer.parseInt(voterData[1]);
                totalSigns += Integer.parseInt(voterData[2]);

            }

                Text text1 = new Text("TOTAL NUMBER OF CHAIRS ..........: " + totalChairs + "\n");
                text1.setFontSize(fontSize);

                Text text2 = new Text("TOTAL NUMBER OF TABLES ..........: " + totalTables + "\n");
                text2.setFontSize(fontSize);

                Text text3 = new Text("TOTAL NUMBER OF PRECINCT SIGNS ..: " + totalSigns + "\n");
                text3.setFontSize(fontSize);

                Paragraph paragraph1 = new Paragraph();
                paragraph1.setTextAlignment(TextAlignment.LEFT);
                paragraph1.setMargins(0,0,0,0);

                Paragraph paragraph2 = new Paragraph();
                paragraph2.setTextAlignment(TextAlignment.LEFT);
                paragraph2.setMargins(0,0,0, 0);

                paragraph1.add(text1).addStyle(normal);
                paragraph1.add(text2).addStyle(normal);
                paragraph1.add(text3).addStyle(normal);
                paragraph2.add("-----------------------------------------------------------------------------------------------------------------------------------------------------------\n").addStyle(normal);

                doc.add(paragraph1);
                doc.add(paragraph2);
        }
        catch (Exception e)
        {
            throw new RuntimeException(e);
        }
    }

    // function that adds data to the report
    private static void getDataForReport(Document doc, String startingID, String endingID, String user) throws IOException
    {
        String[] voterData = new String[20];

        Style normal = new Style();
        PdfFont font = PdfFontFactory.createFont(FontConstants.COURIER);
        normal.setFont(font).setFontSize(fontSize);

        if (Objects.equals(endingID, ""))
        {
            endingID = startingID;
        }

        try
        {
            Class.forName("");
            String url = "";
            Connection conn = DriverManager.getConnection(url);

            PreparedStatement stmt = conn.prepareStatement("SELECT " +
                    "[].[], " +
                    "[].[], " +
                    "[].[], " +
                    "[].[], " +

                    "[].[], " +
                    "[].[], " +
                    "[].[], " +
                    "[].[], " +

                    "[].[], " +
                    "[].[], " +
                    "[].[], " +

                    "[].[], " +
                    "[].[], " +
                    "[].[], " +

                    "[].[], " +
                    "[].[], " +

                    "[].[], " +
                    "[].[], " +
                    "[].[], " +
                    "[].[] " +

                    "FROM [] " +
                    "INNER JOIN [] " +
                    "ON [].[] = [].[]" +
                    "WHERE ["+ columnName +"] BETWEEN '" + startingID + "' AND '" + endingID + "' " +
                    "ORDER BY [" + sortingMetric + "] ASC");

            ResultSet rs = stmt.executeQuery();

            while (rs.next())
            {
                voterData[0] = rs.getString(1);  
                voterData[1] = rs.getString(2);
                voterData[2] = rs.getString(3);   
                voterData[3] = rs.getString(4);   

                voterData[4] = rs.getString(5);  
                voterData[5] = rs.getString(6);  
                voterData[6] = rs.getString(7);  
                voterData[7] = rs.getString(8);  

                voterData[8] = rs.getString(9);   
                voterData[9] = rs.getString(10);  
                voterData[10] = rs.getString(11);

                voterData[11] = rs.getString(12);
                voterData[12] = rs.getString(13); 
                voterData[13] = rs.getString(14);

                voterData[14] = rs.getString(15);
                voterData[15] = rs.getString(16);

                voterData[16] = rs.getString(17);
                voterData[17] = rs.getString(18);
                voterData[18] = rs.getString(19); 
                voterData[19] = rs.getString(20);

                for (int i = 0; i < 17; i++)
                {
                    if (voterData[i] == null)
                    {
                        voterData[i] = "";
                    }
                }

                if (Objects.equals(voterData[9], "0"))
                {
                    voterData[9] = "";
                }

                if (!Objects.equals(voterData[9], ""))
                {
                    voterData[9] = voterData[9].substring(0,3) + "-" + voterData[9].substring(3,6) + "-" + voterData[9].substring(6);
                }

                if (Objects.equals(voterData[10], "0"))
                {
                    voterData[10] = "";
                }

                if (!Objects.equals(voterData[10], ""))
                {
                    voterData[10] = voterData[10].substring(0,3) + "-" + voterData[10].substring(3,6) + "-" + voterData[10].substring(6);
                }

                if (Objects.equals(voterData[12], "0"))
                {
                    voterData[12] = "";
                }

                if (!Objects.equals(voterData[12], ""))
                {
                    voterData[12] = voterData[12].substring(0,3) + "-" + voterData[12].substring(3,6) + "-" + voterData[12].substring(6);
                }

                if (Objects.equals(voterData[13], "0"))
                {
                    voterData[13] = "";
                }

                if (!Objects.equals(voterData[13], ""))
                {
                    voterData[13] = voterData[13].substring(0,3) + "-" + voterData[13].substring(3,6) + "-" + voterData[13].substring(6);
                }


                String address = voterData[4] + " " + voterData[5] + " " + voterData[6] + " " + voterData[7];

                //                      precode                                 prename                              machine count                      address / location / additional                       contact                           phone 1                             phone 2                        delivery                               inventory
                Text text1 = new Text(CommonFunctions.addPaddingR(voterData[0], 7) + CommonFunctions.addPaddingR(voterData[1], 20) + CommonFunctions.addPaddingR(voterData[16], 2) + CommonFunctions.addPaddingR(voterData[2], 56) + CommonFunctions.addPaddingR(voterData[8], 20) + CommonFunctions.addPaddingR(voterData[9], 14) + CommonFunctions.addPaddingR(voterData[10], 14) + CommonFunctions.addPaddingR(voterData[14], 17) + "CH " + voterData[17] + "\n");
                text1.setFontSize(fontSize);

                Text text2 = new Text(                                                                                                         CommonFunctions.addPaddingR(address  + " " + voterData[3], 56) +                     CommonFunctions.addPaddingR(voterData[11], 20) + CommonFunctions.addPaddingR(voterData[12], 14) + CommonFunctions.addPaddingR(voterData[13], 14) + CommonFunctions.addPaddingR(voterData[15], 17) + "TB " + voterData[18] + "\n");
                text2.setFontSize(fontSize);

                Text text3 = new Text(                                                                                                         CommonFunctions.addPaddingR(CommonFunctions.getAdditionalPrecincts(voterData[2], voterData[1]), 121) + "PS " + voterData[19] + "\n");
                text3.setFontSize(fontSize);

                Paragraph paragraph1 = new Paragraph();
                paragraph1.setTextAlignment(TextAlignment.LEFT);
                paragraph1.setMargins(0,0,0,0);

                Paragraph paragraph2 = new Paragraph();
                paragraph2.setTextAlignment(TextAlignment.LEFT);
                paragraph2.setMargins(0,0,0,(float)156.5);

                Paragraph paragraph3 = new Paragraph();
                paragraph3.setTextAlignment(TextAlignment.LEFT);
                paragraph3.setMargins(0,0,0,0);

                paragraph1.add(text1).addStyle(normal);
                paragraph2.add(text2).addStyle(normal);
                paragraph2.add(text3).addStyle(normal);
                paragraph3.add("-----------------------------------------------------------------------------------------------------------------------------------------------------------\n").addStyle(normal);

                if (count == 10)
                {
                    Text blankSpace = new Text("\n");

                    paragraph3.add(blankSpace);
                }

                doc.add(paragraph1);
                doc.add(paragraph2);
                doc.add(paragraph3);
                count++;

                // adds header to the top of each new page
                if (count == 11)
                {
                    page++;
                    String paddedPage = "";

                    if (Integer.toString(page).length() == 1)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 13)+page;
                    }

                    if (Integer.toString(page).length() == 2)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 12)+page;
                    }

                    if (Integer.toString(page).length() == 3)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 11)+page;
                    }

                    if (Integer.toString(page).length() == 4)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 10)+page;
                    }

                    if (Integer.toString(page).length() == 5)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 9)+page;
                    }

                    if (Integer.toString(page).length() == 6)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 8)+page;
                    }

                    if (Integer.toString(page).length() == 7)
                    {
                        paddedPage = CommonFunctions.addPaddingR("PAGE", 7)+page;
                    }

                    Text headerText1 = new Text("REPORT DATE  "+ dateToday +"                                        FCCO VOTER PROCESSING SYSTEM                                                 " + paddedPage + "\n");
                    headerText1.setFontSize(fontSize); //                                                                                         |

                    // Files ID to left side and Report Time to right side
                    Text headerText2 = new Text("USER  "+CommonFunctions.addPaddingR(user, 61)+"VOTING LOCATION LIST                                                     TIME  "+timeToday + "\n");
                    headerText2.setFontSize(fontSize); //                                                     |

                    // add date range
                    Text headerText3 = new Text("INCLUDING ........:  "+ headerIncluded +" FROM "+startingID+" TO "+endingID+" by " + distType + "\n");
                    headerText3.setFontSize(fontSize);

                    // column headings
                    Text headerText4_1 = new Text("PRECINCT                   # LOCATION / PRECINCTS                                    CONTACT             PHONE #1      PHONE #2      DELIVERY     INVENTORY\n");
                    headerText4_1.setFontSize(fontSize);

                    Paragraph header1 = new Paragraph();
                    header1.setTextAlignment(TextAlignment.LEFT);

                    Paragraph header2 = new Paragraph();
                    header2.setTextAlignment(TextAlignment.LEFT);

                    header1.add(headerText1).addStyle(normal);
                    header1.add(headerText2).addStyle(normal);
                    header1.add(headerText3).addStyle(normal);
                    header1.add(headerText4_1).addStyle(normal);
                    header2.add("-----------------------------------------------------------------------------------------------------------------------------------------------------------").addStyle(normal);

                    doc.add(header1);
                    doc.add(header2);

                    count = 0;
                }
            }
        }
        catch (Exception e)
        {
            throw new RuntimeException(e);
        }
    }
}
